...
void CacheL1::readAddr(long long inAddr, long inCyc) { 
  ...
  if(searchTag(inAddr, set, way)) {
    if(g_Debug) cout<<" L1 Cache hit 0x"<<hex<<inAddr<<dec<<endl;
    mHits++;
    mLatency+=mHitLat;
  } else {
    if(g_Debug) cout<<" L1 Cache miss 0x"<<hex<<inAddr<<dec<<endl;
...
void CacheL1::sendMemReq(long long inAddr, int inSet, int inWay, int inDirty, long inCyc) {
  reloadPkt pkt;
  ...
  if(g_Debug) cout<<" L1->L2 Req for Addr: 0x"<<hex<<inAddr<<" Set: 0x"<<inSet<<" Way: 0x"<<inWay<<dec<<endl;
}

void CacheL1::processActiveReloads(long inCyc) {
